  ...
  return (
    <main className="min-h-screen bg-cream text-ink">
      ...
      <section className="border-b-2 border-ink bg-ink text-cream">
        <div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
          <Reveal>
            <p className="font-mono text-sm tracking-widest text-cream/60">Three themes</p>
        ...
        <div className="mx-auto max-w-5xl space-y-12 px-6 py-20 md:py-28">
          {weeks.map((w, i) => (
            <Reveal key={w.n} delay={(i % 2) * 0.15} amount={0.2}>
              <article className="rounded-3xl border-2 border-ink bg-cream p-8 shadow-[8px_8px_0_0_var(--ink)] md:p-12">
                <div className="flex items-start justify-between gap-6">
                  <span className={`rounded-full px-4 py-1 font-mono text-xs font-bold ${w.accent}`}>Week {w.n}</span>
                ...
                <div className="mt-8 grid gap-8 md:grid-cols-2">
                  <div>
                    <p className="font-mono text-xs tracking-widest text-muted-foreground">What we cover</p>
                  ...
                  <div>
                    <p className="font-mono text-xs tracking-widest text-muted-foreground">How it feels</p>
          ...
          <Reveal direction="scale" amount={0.2}>
            <article className="grid gap-6 rounded-3xl border-2 border-ink bg-sun p-8 md:grid-cols-3 md:p-12">
              <div className="md:col-span-1">
                <p className="font-mono text-xs tracking-widest text-ink/60">Built-in support</p>
                <h3 className="mt-2 font-display text-3xl font-black">However you learn.</h3>
              </div>
              <ul className="md:col-span-2 space-y-4 text-ink/80">
                <li>
                  <span className="font-display text-lg font-black">Guided notes.</span> Every lesson comes with
                  structured, pre-built notes that simplify complex ideas.
                </li>
                <li>
                  <span className="font-display text-lg font-black">Out-of-class help.</span> Any Camp Director will
                  meet one-on-one with any camper — for camp topics, school homework, or anything in between.
                </li>
                <li>
                  <span className="font-display text-lg font-black">Stretch material.</span> Curious campers get harder
                  problem sets, project ideas, and reading recommendations to explore between sessions.
                </li>
              </ul>
            </article>
          </Reveal>
